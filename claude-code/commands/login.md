...
The loopback wait times out after 60 seconds. If the binary exits non-zero, the stderr will explain why (timeout, state mismatch, browser-open failure, etc.). The user re-runs `/fancysauce-savings:login` to retry.
...
Tell the user briefly what happened (signed in, backfill accepted/not) and point them at `/fancysauce-savings:upload-history --status` if they want to monitor backfill, or `/fancysauce-savings:upload-history` to start one later.
