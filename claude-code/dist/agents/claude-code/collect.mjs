...
var SubagentCursor = class {
  ...
  constructor(dir) {
    this.dir = dir;
    this.path = join16(dir, "transcript_cursor.json");
    this.tmpPath = `${this.path}.tmp`;
  }
  async read() {
    try {
...
var TranscriptTail = class {
  ...
  constructor(stateDir, options = {}) {
    ...
    const root = options.transcriptRoot ?? join17(homedir4(), ".claude", "projects");
  ...
  async tail(sessionId, transcriptPath, sequenceBase, persist) {
    ...
    const cursorPath = join17(cursorDir, "transcript_cursor.json");
    let release;
    try {
    ...
    try {
      ...
      const sessionDir = join17(dirname6(transcriptPath), basename2(transcriptPath, ".jsonl"));
      ...
      for (const subagentPath of subagentPaths) {
        ...
        const subCursorDir = join17(this.cursorDir(sessionId), "subagents", agentId);
        let read;
        try {
  ...
  cursorDir(sessionId) {
    return join17(this.stateDir, "sessions", sessionId);
  }
...
async function discoverSubagentTranscripts(sessionDir) {
  const subDir = join17(sessionDir, "subagents");
  let entries;
  try {
  ...
  return entries.filter((e) => e.startsWith("agent-") && e.endsWith(".jsonl")).map((e) => join17(subDir, e));
}
...
function readStdin() {
  try {
    const buf = readFileSync3(0, "utf8");
