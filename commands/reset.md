...
Run `node "${CLAUDE_PLUGIN_ROOT}/dist/shared/bin/reset.mjs"` with any args the user passed (e.g., `--all`, `--queue-only`, `--no-confirm`) via the Bash tool, or PowerShell where that is the registered shell.
